...
### 3.1 (백엔드) Django Session 활용 1
* 쿠키와 세션의 개념 정의
  * 쿠키 : 클라이언트 단에서 가지고 있는 접속 정보
  * 세션 : 서버에서 가지고 있는 접속 정보
* 각 타입별 사용자 추가 url 구현

### 3.2 (백엔드) Django Session 활용 2
* 로그인 페이지 및 세션 저장 로직 추가
* 세션을 활용해서 로그인 타입별(사장, 배달기사, 유저 등) 페이지 권한 오류 추가

### 3.3 (인프라) Docker 개요
* 가상환경 및 docker 사용 이유
  1. 개발 환경을 그대로 복제해서 같은 환경에서 배포하고 싶다.
  2. 같은 서버에서 개발환경을 여러개로 분리하고 싶다
* 가상환경과 VS docker
  1. 가상환경보다 docker 가 가벼움(컨테이너를 통한 격리)
  2. 컨테이너별로 자원을 유동적으로 사용할 수 있다. 
* docker-compose : 여러개의 docker container 를 한번에 배포할 수 있는 툴

### 3.4 (인프라) Django Docker Build
